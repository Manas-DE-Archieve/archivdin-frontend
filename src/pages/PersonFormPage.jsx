...
export default function PersonFormPage() {
  ...
  const handleAutoExtract = async (e) => {
    ...
    try {
      ...
      setForm(prev => ({
        ...prev,
        ...Object.fromEntries(Object.entries(data).map(([k, v]) => [k, v || prev[k]])),
  ...
  const submit = async (force = false) => {
    ...
    try {
      ...
      const res = isEdit ? await personsApi.update(id, payload) : await personsApi.create(payload);
      ...
      <div className="flex items-start justify-between mb-7">
        ...
        <div className="mt-8">
          ...
          <button
            ...
            disabled={extracting}
            className="btn-primary !bg-indigo-600 hover:!bg-indigo-700 !shadow-lg transition-all"
          >
            {extracting ? '⏳ Читаю...' : '🪄 Заполнить из скана'}
          </button>
        </div>
      </div>
      <div className="card p-7 shadow-card-lg space-y-6 relative">
        {extracting && (
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><F name="birth_year" label={t('person.birthYear')} type="number" /><F name="death_year" label={t('person.deathYear')} type="number" /></div>
        ...
        <div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4"><F name="arrest_date" label={t('person.arrestDate')} type="date" /><F name="sentence_date" label={t('person.sentenceDate')} type="date" /></div>
        ...
        {error && (<div className="flex items-center gap-2 text-sm text-red-600 bg-red-50 border border-red-100 rounded-lg px-3 py-2.5"><span className="shrink-0">⚠</span><span>{error}</span></div>)}
        <div className="flex gap-3 pt-2 border-t border-slate-100">
          <button type="button" onClick={() => navigate(-1)} className="btn-outline flex-1 justify-center">{t('person.cancel')}</button>
          <button type="button" onClick={() => submit(false)} disabled={loading || !form.full_name} className="btn-primary flex-1 justify-center">
            {loading ? (<span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin" />{t('common.loading')}</span>) : t('person.save')}
          </button>
        </div>
      </div>
    </div>
